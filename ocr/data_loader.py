...
class ScrollLineMixedDatasetIterable(IterableDataset):
    def __init__(self, tokenizer, image_size):
        super().__init__()
        self.tokenizer = tokenizer
        self.image_size = image_size
    ...
    def line_iterator(self):
        while True:
            #tokens_bible, seg_bible, scrolls_bible, lines_bible = self.generator.generate_passages_scrolls(100, skip_char_seg=False)
            tokens_grams, seg_grams, scrolls_grams, lines_grams = self.generator.generate_passages_scrolls(100, skip_char_seg=False)
...
class ScrollLineNoiseDatasetIterable(IterableDataset):
    def __init__(self, 
                 tokenizer: Tokenizer, 
                 image_size: tuple, 
                 warp_strength: tuple[float, float] = (2, 10),
            ...
            for level, (p, w, c) in enumerate(zip(self.perlin_strength, self.warp_strength, self.cutout_size, strict=True)):
                ...
                for i in range(scrolls.shape[0]):
                    ...
                    for idx in range(n_indicies):
                        ...
                        #image_modified = Image.fromarray(modified_image_lines[idx])
                        #image_modified.save("original_noise.png")
                        image = image_creator.pad(image, self.image_size)
                        image = self.transform(image)
                        #to_pil = ToPILImage()
