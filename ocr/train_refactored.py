...
def save_scripts(destination):
    ...
    for script in ["train_refactored.py", "ocr_model.py", "data_loader.py"]:
...
class OCRTrainer:
    ...
    def _load_datasets(self):
        ...
        self.train_loader = DataLoader(
            data_loader_refactored.NoiseMixedDatasetIterable(self.tokenizer, self.image_size),
            batch_size=self.batch_size, shuffle=False,
            collate_fn=lambda b: ocr_collate_fn(b, self.tokenizer.pad_token_id)
        )
        self.val_loader = DataLoader(
            data_loader_refactored.ScrollLineDataset(os.path.join(data_base, 'test_images_ocr', 'tokens.parquet'),
                                                     os.path.join(data_base, 'test_images_ocr'), self.tokenizer),
            batch_size=self.batch_size, shuffle=False,
            collate_fn=lambda b: ocr_collate_fn(b, self.tokenizer.pad_token_id)
        )
        self.ngram_loader = DataLoader(
            data_loader_refactored.ScrollLineDataset(os.path.join(data_base, 'n_gram_images', 'tokens.parquet'),
                                                     os.path.join(data_base, 'n_gram_images'), self.tokenizer),
            batch_size=self.batch_size, shuffle=False,
            collate_fn=lambda b: ocr_collate_fn(b, self.tokenizer.pad_token_id)
        )
        self.test_loader = DataLoader(
            data_loader_refactored.ScrollLineDatasetWithPadding(os.path.join(test_base, 'tokens.parquet'),
